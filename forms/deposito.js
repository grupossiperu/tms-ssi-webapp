 ...
 * del viaje, por ejemplo), pide monto, persona que deposita y medio de
 * pago, y lo SUMA de inmediato al "Monto a depositar" que se muestra en
 * la tabla. No marca ni desmarca el check "Depositado".
 * -------------------------------------------------------------------------
 */
const FormDeposito = {
    ...
    async function cargar() {
      ...
      filas.forEach(function (f) {
        ...
        btnMas.addEventListener('click', async function (ev) {
          ...
          const persona = window.prompt('Nombre de quién realiza el depósito adicional:', '');
          if (persona === null || persona.trim() === '') {
            mostrarMensaje('Ingrese el nombre de la persona que realiza el depósito.', 'error');
            return;
          }

          const medioTxt = window.prompt('Medio de pago (EFECTIVO, TRANSFERENCIA o YAPE):', 'TRANSFERENCIA');
          if (medioTxt === null) return;
          const medio = String(medioTxt).trim().toUpperCase();
          if (['TRANSFERENCIA', 'YAPE', 'EFECTIVO'].indexOf(medio) === -1) {
            mostrarMensaje('Medio de pago inválido. Use EFECTIVO, TRANSFERENCIA o YAPE.', 'error');
            return;
          }
